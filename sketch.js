...
let tempDiff = [10,0,-30,5,20,250,-30,20,-5,0,50,-2,250,
                ...
                ,0,0,10,-10,11,59,250,-20,0,10,20,40,9,99,-20,250,
                -10,4,9,20,-20,5,12,0,0,50,250,30,0,20,30,10,-30,-20,0,-10,250];

function setup() {
  createCanvas(1470,750);
  smooth();
  img1 = loadImage('WechatIMG796.png');
  img2 = loadImage('WechatIMG797.png');
...
function draw() {
    ...
    drawGrid();
    drawLines();
    image(img1, 1050, 15, img1.width / 4, img1.height / 4);
    image(img2, 1050, 350, img2.width / 4, img1.height / 4);
...
function drawLines() {
	for (let j=0; j<800; j+= cellSize)
	{
	  for (let i=0; i<800; i+= cellSize)
	  {
	let cellNum = (j/cellSize * 800/cellSize) + i/cellSize;
